...
String txtName = "frec55muest500.txt";
String[] txtBuffer;
float[] dataBuffer = new float[dataSize];

PitchDetector realPitch = new Yin(sampleRate,dataSize);
float pitchVal;
// Variables de algoritmo YIN


void setup(){
  txtBuffer = loadStrings(txtName);
  //dataSize = txtBuffer.length
  for(int i = 0; i < dataSize; i++){
    dataBuffer[i]=float(txtBuffer[i]);
  }
  
}

void draw(){
  println("tono = "+(realPitch.getPitch(dataBuffer)).getPitch());
}
